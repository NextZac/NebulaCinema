...
const animationStyle = computed(() => ({
    transform: isOpen.value ? 'translateY(0px)' : alignmentTransformAnimation.value,
    opacity: isOpen.value ? '1' : '0',
    maxHeight: isOpen.value ? '100vh' : alignmentHeightAnimation.value,
    maxWidth: isOpen.value ? '100vw' : alignmentWidthAnimation.value,
    overflow: 'hidden',
...
<template>
    <div
        ...
        <button @click="toggleDropdown" type="button"
            ...
            <ChevronDown
                :class="['w-4 h-4 mr-[12px] transition-transform duration-100', isOpen ? iconRotationActive : iconRotation]">
            </ChevronDown>
        </button>
