...
<template>
    <div ref="dropdownRef"
        :class="[props.class, alignmentClasses, 'absolute z-20 my-1 w-min bg-brand-975 border border-brand-900 shadow-lg rounded-md text-base sm:text-sm']"
        :style="dropdownStyle">
        <ul tabindex="-1"
            ...
            <a v-for="(option, index) in props.options" :key="index"
                :class="[colorSelect(index), 'text-body select-none relative text-nowrap flex items-center']" :href="option.href">
                {{ option.title }}
            </a>
        </ul>
    </div>
